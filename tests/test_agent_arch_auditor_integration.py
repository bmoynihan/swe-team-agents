...
def test_validator_knows_agent_arch_auditor_optional_integration() -> None:
    ...
    assert "agent-arch-auditor.agent.md" in text
    assert "agent-arch-auditor.instructions.md" in text
    assert (
        '"agentArchitectureAuditReport": "docs/agents/agent-architecture-audit-report.md"' in text
    )


def test_validator_checks_tracked_pycache_only() -> None:
    text = (ROOT / "scripts" / "ci" / "check_agent_system.py").read_text(encoding="utf-8")

    assert 'git", "ls-files", "-z' in text
    assert "Remove committed __pycache__ directories" in text
