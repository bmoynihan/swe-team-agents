...
def _render_lock_note(report_artifact: dict[str, Any] | None) -> str:
    ...
    if report_fallback or snapshot_fallback:
        lines.extend(
            [
                "The preferred `docs/agents` report target could not be updated "
                "directly, so this run wrote a fallback artifact.",
                "",
...
def run_pipeline(
    ...
    if run_smoke_test:
        ...
        smoke_output = _run_command(
            [node_command, "./scripts/local_smoke_test.mjs"], cwd=package_root
        )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            "Preferred docs/agents report path. The runner falls back safely if it is write-locked."
        ),
    )
    parser.add_argument(
        "--status",
