...
def test_emit_package_falls_back_when_report_target_is_locked(monkeypatch: pytest.MonkeyPatch) -> None:
    module = load_module()
    sandbox_root = Path(tempfile.mkdtemp(prefix="emitter-", dir=ROOT / ".tmp"))
    ...
    try:
        ...
        assert run_snapshot["actualPath"].endswith("docs/agents/runs/test-run/copilot-agent-conversion-report.generated.md")
