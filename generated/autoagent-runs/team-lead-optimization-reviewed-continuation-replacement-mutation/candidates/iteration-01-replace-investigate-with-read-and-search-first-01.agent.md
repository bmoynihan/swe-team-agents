...
1. Intake
2. Research -> `read_file` + `grep_search` first; `repo-researcher` only if needed
3. Spec -> `spec-writer`
4. Implement -> `implementer`
5. Validate -> `test-engineer`
6. Quality Gate -> `quality-gate`
7. Release Prep -> `release-manager` if release evidence is required
8. Done -> ready for human review

If the Quality Gate fails, return to Implement and iterate.
...
All specialist requests and results must include a short human summary and one machine-parseable JSON object.
